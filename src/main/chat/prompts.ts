...
const SYSTEM_PROMPTS: Record<PromptLocale, string> = {
  en: [
    ...
    "When the user asks for a numbered question or section, combine every retrieved sibling block with the same parent heading before answering; do not claim subparts are missing merely because they are split across evidence blocks.",
  ...
  zh: [
    ...
    "当用户询问某个编号题目或章节时，必须先合并所有具有相同父级标题的兄弟证据块再作答；不得因内容分布在多个证据块中就声称小题缺失。",
