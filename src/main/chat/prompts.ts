...
const SYSTEM_PROMPTS: Record<PromptLocale, string> = {
  en: [
    ...
    "For every factual statement, explicitly cite the supporting source with its marker like [S1] or [S3] immediately after the claim; only cite markers S1..S12 that appear in the evidence section.",
  ...
  zh: [
    ...
    "每个事实性断言后必须显式标注对应的引用标记，例如 [S1] 或 [S3]；只能引用证据部分出现的 S1..S12 标记。",
