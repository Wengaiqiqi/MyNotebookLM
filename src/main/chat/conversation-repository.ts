...
export class ConversationRepository {
 ...
 createConversation(i:{id:string;projectId:string;title:string;createdAt:string}){this.db.prepare("INSERT INTO conversations(id,project_id,title,created_at,updated_at,deleted_at,archived_at) VALUES(?,?,?,?,?,NULL,NULL)").run(i.id,i.projectId,i.title,i.createdAt,i.createdAt);return this.getConversation(i.projectId,i.id)!;}
 getConversation(p:string,id:string):ConversationDto{const r=this.db.prepare("SELECT * FROM conversations WHERE id=? AND project_id=?").get(id,p) as any;if(!r) throw new Error("conversation not found");return this.mapConversation(r);}
 listConversations(p:string){return (this.db.prepare("SELECT * FROM conversations WHERE project_id=? AND deleted_at IS NULL AND archived_at IS NULL ORDER BY updated_at DESC").all(p) as any[]).map(r=>this.mapConversation(r));}
 ...
 updateUserMessage(i:{projectId:string;id:string;content:string;updatedAt:string}){const m=this.getMessage(i.projectId,i.id);if(!m||m.role!=="user")throw new Error("message not editable");this.db.prepare("UPDATE messages SET content=?,updated_at=? WHERE id=?").run(i.content,i.updatedAt,i.id);return this.getMessage(i.projectId,i.id)!;}
 ...
 completeAssistantMessage(i:Omit<Base,"createdAt">&{usage:{inputTokens:number;outputTokens:number;totalTokens:number};completionReason?:string;provider?:string;profileId?:string;model?:string;updatedAt:string}){return this.db.transaction(()=>{const m=this.getMessage(i.projectId,i.id);if(m?.role!=="assistant"||m?.state!=="streaming")throw new Error("message not completable");this.db.prepare("UPDATE messages SET content=?,state='completed',usage_json=?,completion_reason=?,provider=COALESCE(?,provider),profile_id=COALESCE(?,profile_id),model=COALESCE(?,model),updated_at=? WHERE id=?").run(i.content??"",JSON.stringify(i.usage),i.completionReason??"stop",i.provider??null,i.profileId??null,i.model??null,i.updatedAt,i.id);return this.getMessage(i.projectId,i.id)!;})();}
 checkpointAssistantContent(i:{projectId:string;messageId:string;content:string;updatedAt:string}){const m=this.getMessage(i.projectId,i.messageId);if(!m||m.role!=="assistant"||m.state!=="streaming")return;this.db.prepare("UPDATE messages SET content=?,updated_at=? WHERE id=? AND state='streaming'").run(i.content,i.updatedAt,i.messageId);}
 cancelAssistantMessage(i:{projectId:string;messageId:string;updatedAt:string}){const m=this.getMessage(i.projectId,i.messageId);if(!m||m.role!=="assistant"||m.state!=="streaming")throw new Error("message not cancellable");this.db.prepare("UPDATE messages SET state='cancelled',completion_reason=COALESCE(completion_reason,'user_abort'),updated_at=? WHERE id=?").run(i.updatedAt,i.messageId);return this.getMessage(i.projectId,i.messageId)!;}
 failAssistantMessage(i:{projectId:string;messageId:string;errorCode:string;updatedAt:string}){const m=this.getMessage(i.projectId,i.messageId);if(!m||m.role!=="assistant"||m.state!=="streaming")throw new Error("message not failable");this.db.prepare("UPDATE messages SET state='failed',error_code=?,updated_at=? WHERE id=?").run(i.errorCode,i.updatedAt,i.messageId);return this.getMessage(i.projectId,i.messageId)!;}
 addCitation(i:{projectId:string;messageId:string;id:string;label:string;sourceId:string;sourceChunkId:string;sourceDisplayName:string;sourceKind:string;locator:unknown;quote?:string;createdAt:string;start?:number}){return this.db.transaction(()=>{const m=this.getMessage(i.projectId,i.messageId);if(!m)throw new Error("message not found");this.own(i.projectId,m.conversationId);this.db.prepare("INSERT INTO message_citations(id,message_id,label,source_id,source_chunk_id,source_display_name,source_kind,locator_json,quote,created_at,start) VALUES(?,?,?,?,?,?,?,?,?,?,?)").run(i.id,i.messageId,i.label,i.sourceId,i.sourceChunkId,i.sourceDisplayName,i.sourceKind,JSON.stringify(i.locator),i.quote??null,i.createdAt,i.start??0);return this.getMessage(i.projectId,i.messageId)!.citations.find(c=>c.id===i.id)!;})();}
