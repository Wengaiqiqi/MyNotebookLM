...
export class MainSourceService {
  ...
  listSources(projectId: string): SourceDto[] { return this.db.prepare("SELECT s.*, sr.original_path AS current_locator, sr.stored_path AS current_stored_path, sr.state AS current_revision_state FROM sources s LEFT JOIN source_revisions sr ON sr.id = COALESCE(s.current_revision_id, (SELECT id FROM source_revisions WHERE source_id=s.id ORDER BY created_at DESC, id DESC LIMIT 1)) WHERE s.project_id = ? AND s.status <> 'deleted' ORDER BY s.updated_at DESC").all(projectId).map((row) => this.source(row as Row)); }
  listTasks(projectId: string): TaskDto[] { return this.db.prepare("SELECT t.*, s.rule_id AS transformation_rule_id FROM tasks t LEFT JOIN transformation_task_snapshots s ON s.task_id=t.id WHERE t.project_id = ? ORDER BY t.created_at DESC").all(projectId).map((row) => this.task(row as Row)); }
