...
export function registerSourceHandlers(ipc: Ipc, service: Service, dialogs?: Dialog): () => void {
  ...
  const safe = async (schema: z.ZodType, call: (input: any) => unknown, input: unknown) => { const parsed = schema.safeParse(input); if (!parsed.success) return validationFailure(); try { const value = await call(parsed.data); return { ok: true as const, value }; } catch { return internalFailure(); } };
