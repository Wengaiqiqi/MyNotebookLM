...
type SourceOpener = (input: { projectId: string; sourceId: string }) => Promise<unknown>;
...
export function registerSourceHandlers(ipc: Ipc, service: Service, dialogs?: Dialog, openSource?: SourceOpener): () => void {
  ...
  ipc.handle(SOURCE_CHANNELS.open, async (_event, input) => { const parsed = source.safeParse(input); if (!parsed.success || !openSource || service.ownsSource?.(parsed.data.projectId, parsed.data.sourceId) === false) return validationFailure(); try { return await openSource(parsed.data); } catch { return internalFailure(); } });
