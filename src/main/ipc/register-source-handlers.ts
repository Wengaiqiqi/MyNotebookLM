...
export function registerSourceHandlers(ipc: Ipc, service: Service, dialogs?: Dialog): () => void {
  ...
  ipc.handle(SOURCE_CHANNELS.listTasks, (_event, input) => { const value = project.parse(input); return z.array(taskDtoSchema).parse(service.listTasks(value.projectId)); });
  ipc.handle(SOURCE_CHANNELS.remove, (_event, input) => safe(source, (value) => service.removeSource?.(value), input));
  ipc.handle(SOURCE_CHANNELS.retry, (_event, input) => safe(source, (value) => service.retryTask?.(value), input));
  ipc.handle(SOURCE_CHANNELS.cancel, (_event, input) => safe(task, (value) => service.cancelTask?.(value), input));
