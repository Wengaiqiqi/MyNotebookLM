...
type Service = {
  ...
  startTask(input: TransformationRunRequest, signal: AbortSignal, onFinished: () => void, onOwnership: (ownership: { taskId: string; owned: boolean }) => void): Promise<TaskDto>;
...
export function registerTransformationHandlers(ipc: Ipc, service: Service): () => void {
  ...
  ipc.handle(TRANSFORMATION_CHANNELS.run, (_event, input) => safe(transformationRunInputSchema, task, input, async (value) => { const controller = new AbortController(); let created!: TaskDto; let ownedTaskId: string | undefined; let ownershipNotified = false; const onFinished = () => { const taskId = ownedTaskId ?? created?.id; if (taskId && controllers.get(taskId) === controller) controllers.delete(taskId); }; const onOwnership = ({ taskId, owned }: { taskId: string; owned: boolean }) => { if (ownershipNotified) return; ownershipNotified = true; ownedTaskId = taskId; if (owned) controllers.set(taskId, controller); else controller.abort(); }; created = await service.startTask(value as TransformationRunRequest, controller.signal, onFinished, onOwnership); return created; }));
