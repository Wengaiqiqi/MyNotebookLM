...
export class LanceStore {
  ...
  async upsert(space: LanceSpace, rows: LanceRow[]): Promise<void> { for (const row of rows) if (row.vector.length !== space.dimension) throw new Error("vector dimension mismatch"); await locked(tableName(space), async () => { const data: StoredRow[] = rows.map(({ locator, ...r }) => ({ ...r, locatorJson: canonical(locator) })); if (data.length) await (await this.table(space)).mergeInsert("chunkId").whenMatchedUpdateAll().whenNotMatchedInsertAll().execute(data); }); }
