...
export class LanceStore {
  ...
  private readonly readyTextIndexes = new Set<string>();
  ...
  private async ensureTextIndex(table: lancedb.Table, space: LanceSpace): Promise<void> {
    const name = tableName(space);
    if (this.readyTextIndexes.has(name)) return;
    const existing = (await table.listIndices()).find(index => index.name === "text_idx");
    const tokenizer = (existing as { indexDetails?: { base_tokenizer?: unknown } } | undefined)?.indexDetails?.base_tokenizer;
    if (!existing || tokenizer !== FTS_TOKENIZER) {
      await table.createIndex("text", {
        config: lancedb.Index.fts({ baseTokenizer: FTS_TOKENIZER, lowercase: true, stem: false, removeStopWords: false, asciiFolding: true, withPosition: true }),
        replace: Boolean(existing),
        name: "text_idx"
      });
    }
    this.readyTextIndexes.add(name);
  }
