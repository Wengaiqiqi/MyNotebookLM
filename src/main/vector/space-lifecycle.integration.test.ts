...
describe("Space lifecycle real restart recovery", () => {
  ...
  it("recovers source_chunks from a managed original through the real file read path", async () => {
    ...
    const ingestion = new IngestionService(pool, c); const lance = await LanceStore.open(path.join(root, "vectors")); const repo = new SpaceRepository(c, undefined, undefined, lance); const shadow = repo.createOrReuse(spec("p", "managed")); await lance.createSpace(shadow);
    const indexing = new IndexingService(c, { embedBatch: async () => [[1, 0]] }, lance); indexing.setChunkRecovery((revisionId) => ingestion.reparseRevision(revisionId));
