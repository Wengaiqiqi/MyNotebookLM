...
export class WorkerPool {
  ...
  constructor(concurrency = defaultWorkerConcurrency(), private readonly workerUrl = new URL("../../workers/ingestion/worker-entry.ts", import.meta.url), private readonly factory: () => PoolWorker = () => new Worker(this.workerUrl) as PoolWorker, durablePayload?: (taskId: string) => DurableWorkerPayload | undefined, private readonly onProgress?: (taskId: string, value: number) => void) { this.concurrency = Math.max(1, concurrency); this.durablePayload = durablePayload; }
