...
export default function Sidebar(props: SidebarProps) {
  ...
  return <aside className="sidebar" aria-label="Projects and navigation">
    ...
    <nav className="project-nav" aria-label={props.projectTitle}>
      ...
      <button type="button" className="secondary-button" onClick={props.onToggleArchived}>{props.archivedLabel}</button>
    ...
    <footer className="sidebar-footer">
      <button className="settings-button" type="button" aria-current={props.view === "settings" ? "page" : undefined} disabled={props.view === "loading"} onClick={props.onSettings}><span aria-hidden="true">⚙</span>{props.settingsLabel}</button>
      ...
      <div className="preference-row" role="group" aria-label="Language"><button type="button" aria-pressed={props.language === "zh-CN"} onClick={() => props.onLanguage?.("zh-CN")}>中文</button><span aria-hidden="true">|</span><button type="button" aria-pressed={props.language === "en"} onClick={() => props.onLanguage?.("en")}>EN</button></div>
      <div className="preference-row" role="group" aria-label="Theme"><button type="button" aria-pressed={props.theme === "light"} onClick={() => props.onTheme?.("light")}>Light</button><span aria-hidden="true">|</span><button type="button" aria-pressed={props.theme === "dark"} onClick={() => props.onTheme?.("dark")}>Dark</button></div>
    </footer>
  </aside>;
}
