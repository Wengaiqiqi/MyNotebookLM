const zhCN = {
  app: {
    name: "MyNotebookLM",
    settings: "设置"
  },
  project: {
    title: "研究项目",
    create: "新建项目",
    rename: "重命名",
    archive: "归档",
    remove: "删除项目",
    emptyTitle: "开始新的研究项目",
    emptyBody: "创建项目，在一处整理资料、笔记与研究。",
    nameLabel: "项目名称",
    menu: "项目操作",
    removeConfirm: "项目将进入可恢复删除状态，你有 30 秒可以撤销。",
    archived: "已归档项目",
    restore: "恢复",
    undo: "撤销删除",
    retryDelete: "重试删除",
    deletionPending: "项目删除正在等待清理。",
    sections: "项目页面",
    search: "搜索项目",
    searchEmpty: "没有匹配的项目。",
    listEmpty: "还没有项目，创建第一个吧。",
    noArchived: "没有已归档的项目。",
    archivedToast: "项目已归档。"
  },
  common: {
    cancel: "取消",
    confirm: "确认",
    language: "语言",
    theme: "主题",
    light: "浅色",
    dark: "深色",
    save: "保存",
    saving: "正在保存…",
    openSettings: "打开设置",
    retry: "重试", add: "添加", edit: "编辑", delete: "删除", close: "关闭", focusHint: "使用 Tab 键移动焦点", loading: "正在加载…"
  },
  model: {
    outputKind: "模型用途",
    textOutput: "文字生成",
    speechOutput: "语音合成（TTS）",
    podcastVoices: "播客音色",
    speakerVoice: "主持人 {{speaker}} 音色",
    selectVoice: "选择音色…",
    voiceId: "供应商提供的音色 ID",
    getVoices: "获取音色",
    fetchingVoices: "获取音色中…",
    noVoiceCatalog: "服务未返回音色列表，请填写供应商提供的音色 ID。",
    enterVoiceId: "填写音色 ID",
    chooseVoice: "从列表选择",
    generation: { title: "生成模型", description: "用于对话、摘要与内容生成" },
    embedding: { title: "嵌入模型", description: "用于本地索引与语义检索" },
    newProfile: "新建{{capability}}",
    newProfileTitle: "新建提供商配置",
    editProfile: "编辑提供商配置",
    editProvider: "编辑提供商配置",
    editModelConfig: "编辑模型配置",
    savedProfile: "已保存配置",
    generationSettingsSaved: "模型高级配置已保存",
    generationScopeHint: "这些设置用于聊天发送、重试、重新生成和继续生成，不改变工作台转换、标题生成或嵌入预算。",
    outputCeilingHint: "服务商已确认最大输出能力：{{value}} tokens；不能提高到此值以上。",
    profileDeleted: "配置已删除",
    deleteProfile: "删除模型配置",
    deleteProfileBody: "将删除配置 {{name}}。使用该配置的路由需要重新选择模型。",
    noProfiles: "还没有配置，点击右上角按钮新建。",
    disabled: "已停用",
    builtIn: "内置",
    builtInHint: "内置本地嵌入模型 · {{dimension}} 维，无需 API Key。",
    builtInSelected: "已选择",
    localModelMode: "本地模型方式",
    localModelBuiltIn: "使用内置模型",
    localModelCustom: "选择本机模型",
    localModelPath: "模型权重或目录",
    localModelPathPlaceholder: "选择模型权重文件或模型目录",
    chooseLocalModel: "选择文件 / 目录",
    provider: "提供商",
    profileName: "配置名称",
    keySaved: "密钥 {{mask}}",
    apiKeyKeep: "留空则保留现有密钥",
    providers: {
      openai: "OpenAI",
      "openai-compatible": "OpenAI Compatible",
      anthropic: "Anthropic",
      gemini: "Gemini",
      ollama: "Ollama",
      local: "内置模型"
    },
    apiAddress: "API 地址",
    apiKey: "API Key",
    contextTokens: "上下文容量（tokens）",
    maxOutputTokens: "最大输出长度（tokens）",
    contextAutomaticHint: "自动值：{{value}}；容量未手动覆盖时使用此值。",
    contextInputOnlyHint: "服务商只提供输入上限：{{value}}；不代表总上下文窗口。",
    outputAutomaticHint: "自动值：{{value}}；回答不要求写满。",
    contextDefaultHint: "保守默认值：{{value}}；服务商未提供该模型的上下文容量，可手动填写。",
    outputDefaultHint: "保守默认值：{{value}}；服务商未提供该模型的输出上限，可手动填写。",
    restoreAutomatic: "恢复自动",
    showApiKey: "显示 API Key",
    hideApiKey: "隐藏 API Key",
    getModels: "获取模型",
    fetching: "正在获取…",
    fetchSuccess: "获取成功",
    model: "模型",
    modelName: "模型名称",
    selectTextModel: "请选择文字生成模型",
    selectSpeechModel: "请选择语音生成模型，可留空",
    selectedModels: "已选择 {{count}} 个模型",
    noDiscoveredModels: "请先点击“获取模型”加载可选模型。",
    chooseModel: "选择模型",
    manualModel: "手动输入模型名称",
    chooseDiscovered: "选择已获取模型",
    validation: {
      apiKey: "请输入 API Key。",
      address: "请输入有效的 HTTP 或 HTTPS API 地址。",
      localModelPath: "请选择模型权重文件或模型目录。",
      positiveTokens: "{{field}}必须是正整数。",
      complete: "请同时配置有效的生成模型与嵌入模型。"
    },
    viewDetails: "详细",
    hideDetails: "收起",
    configuredModels: "共 {{count}} 个模型",
    modelsCount: "个模型",
    chooseProvider: "选择提供商…",
    selectProviderFirst: "请先选择提供商…",
    modelsList: "包含模型",
    deleteProviderBody: "确定要删除提供商“{{name}}”及其包含的 {{count}} 个模型配置吗？",
    errors: { request: "模型服务请求失败。" }
  },
  onboarding: {
    title: "欢迎使用 MyNotebookLM",
    subtitle: "配置生成模型与嵌入模型后，即可开始导入资料、提问与研究。凭据仅保存在本机。",
    finish: "完成并开始使用",
    skip: "稍后配置模型",
    skipHint: "也可以先跳过，稍后在设置中配置。",
    readySummary: "模型已就绪，点击完成即可开始。",
    localCredential: "凭据仅保存在本机"
  },
  settings: {
    title: "设置",
    subtitle: "管理模型、数据与应用偏好",
    back: "返回",
    general: "通用",
    languageAppearance: "语言与外观",
    themeHint: "界面配色随主题即时切换",
    modelServices: "模型服务",
    dataIndex: "数据与索引",
    loadError: "无法加载模型设置，请检查应用连接后重试。"
  },
  routing: {
    podcastHint: "添加文字生成模型和 TTS 模型。文字模型生成双人对话稿，TTS 模型合成音频；同类模型按排列顺序回退。",
    title: "任务路由",
    description: "为每类任务选择首选模型与有序回退链路。",
    task: "任务",
    orderedRoute: "有序模型路由",
    fallbackRule: "优先使用第一个可用模型，仅在可恢复失败时使用备用配置。",
    embeddingRule: "嵌入仅使用一个配置",
    embeddingProfile: "嵌入配置",
    chooseEmbeddingProfile: "选择嵌入配置",
    emptyRoute: "请添加模型路由",
    addFirstProfile: "添加首个配置",
    addFallback: "添加回退",
    fallbackProfile: "添加回退配置",
    noAvailableProfiles: "没有可用配置",
    saveRoute: "保存路由",
    saved: "路由已保存",
    moveUp: "上移 {{name}}",
    moveDown: "下移 {{name}}",
    remove: "移除 {{name}}",
    fallbackHistory: "回退历史",
    noFallbacks: "尚无路由尝试记录。",
    previousPage: "上一页",
    nextPage: "下一页",
    page: "第 {{page}} 页",
    states: { started: "已开始", completed: "已完成", failed: "失败", cancelled: "已取消" },
    tasks: { chat: "对话", "note-title": "笔记标题", summary: "摘要", "key-points": "要点", qa: "选择题测验", "custom-transformation": "自定义转换", podcast: "播客", embedding: "嵌入" }
  },
  vector: {
    eyebrow: "数据与索引",
    healthy: "正常",
    healthyBody: "当前 Space 已为 {{count}} 个内容建立索引。",
    unavailableBody: "暂时无法访问索引，请前往模型服务配置嵌入模型或重建索引。",
    corruptBody: "索引校验失败，需要重建当前 Space。",
    buildingBody: "正在构建新的嵌入 Space，当前操作完成后才会切换。",
    failedBody: "Space 构建失败，请重试。",
    building: "正在构建",
    progress: "索引任务进度",
    rebuilt: "索引已更新。",
    configureModel: "前往模型服务",
    optimize: "优化索引",
    rebuild: "重建索引",
    migrate: "迁移嵌入 Space",
    cancelTask: "取消索引任务",
    previousSpaceKept: "迁移失败时，之前的嵌入 Space 仍保持启用。",
    migrationTitle: "迁移嵌入 Space？",
    migrationBody: "将使用 {{profile}} 构建新的 Space。完成校验前不会替换当前 Space。",
    migrationRunning: "正在迁移嵌入 Space",
    migrationRunningBody: "正在构建并校验新的 Space，当前 Space 仍可用。",
    migrationFailed: "嵌入 Space 迁移失败",
    migrationFailedBody: "新的 Space 未启用，请检查模型配置后重试。",
    retryMigration: "重试迁移",
    noProject: "请选择一个研究项目后管理其索引。",
    states: { healthy: "索引正常", unavailable: "索引不可用", corrupt: "索引需要修复", building: "正在建立索引", failed: "索引建立失败" }
  },
  errors: {
    podcastRouteMissing: "请在设置 → 任务路由 → 播客中添加至少一个文字生成模型和一个 TTS 模型。",
    podcastScriptInvalid: "生成的播客对话稿格式不正确或不包含两位说话者，请重试或更换文字模型。",
    podcastSpeechUnsupported: "此提供商暂不支持播客语音合成，请使用 OpenAI、OpenAI 兼容或 Gemini 的 TTS 模型。",
    podcastVoicesRequired: "请在运行转换中为两位主持人选择或填写音色。",
    podcastAudioUnavailable: "播客音频无法读取或播放，请重试。",
    auth: "身份验证失败，请检查 API Key 和提供商权限。",
    authentication: "身份验证失败，请检查 API Key 和提供商权限。",
    authorization: "访问被拒绝，请检查 API Key 权限和提供商账户。",
    builtInModelImmutable: "内置嵌入模型不可编辑，请将它直接选为嵌入路由。",
    cancelled: "请求已取消，请重试。",
    interrupted: "应用重启后恢复了中断任务，请重试。",
    conflict: "数据已发生变化，请重新加载后重试。",
    configuration: "请检查提供商地址、API Key 和模型配置后重试。",
    credentialBinding: "已保存的密钥与当前提供商地址不匹配，请重新输入 API Key。",
    internal: "操作未能完成，请重试。",
    invalidResponse: "提供商返回了无效响应，请检查其服务状态后重试。",
    indexUnavailable: "索引暂时不可用，请重试或重建。",
    taskConflict: "已有索引任务正在运行，请稍后再试。",
    embeddingProfileUnavailable: "嵌入模型不可用，请检查模型设置后重试。",
    embeddingRejected: "嵌入模型拒绝了部分内容（通常超出单条输入上限）。系统已自动拆分重试，请重新导入；若仍失败请检查文件内容。",
    unsupportedFormat: "不支持此文件格式，请选择 PDF、DOCX、PPTX、XLSX、TXT、Markdown 或 CSV。",
    unsafeInput: "来源校验失败，请检查文件或网址后重试。",
    generationProfileMissing: "尚未配置生成模型，请打开模型服务设置。",
    chatArchived: "此对话已归档，请新建或恢复一个对话。",
    chatSendInFlight: "已有回答正在生成，请等待完成或停止后重试。",
    chatRegenerateSuperseded: "此回答已被更新，无法再次生成。",
    chatCancelled: "回答已停止。",
    providerFailure: "模型服务请求失败，请检查服务状态后重试。",
    providerIncomplete: "模型服务未完成回答，请重试。",
    responseTooLarge: "模型服务的响应超过本次允许的大小，回答可能已中断。请重试或降低最大输出长度。",
    outputLimitEmpty: "模型用尽了本次输出上限但没有产生可见正文（通常发生在思考过程占满额度时）。可提高最大输出长度或关闭思考后重试。",
    generationLimitsConflict: "上下文容量与最大输出长度冲突：按当前容量最多只能设置 {{limitTokens}} tokens 输出。请降低输出长度或提高上下文容量。",
    continueEvidenceMissing: "原回答引用的资料在当前上下文配置下无法完整保留，请提高上下文容量或重新生成。",
    citationOpenFailed: "无法打开来源，请检查来源文件或网址。",
    citationDetailFailed: "无法读取引用原文，请稍后重试。",
    sourceUnavailable: "来源不可用，请重新导入或检查来源。",
    notFound: "找不到请求的资源，请重新加载设置。",
    modelCapability: "此提供商或模型不支持所需能力，请选择其他模型。",
    modelNotFound: "未找到该模型，请重新获取模型或输入有效的模型名称。",
    modelProfileNotFound: "此模型配置已不存在，请重新加载设置并选择其他配置。",
    generationContextLimit: "上下文容量不能超过服务商已确认的上限。",
    generationOutputLimit: "最大输出长度不能超过服务商已确认的上限（{{limitTokens}} tokens）。",
    continueUnavailable: "这条回答无法继续生成，请重新生成。",
    continueConflict: "这条回答已被其他请求更新，请刷新后重试。",
    continueSourceUnavailable: "原回答引用的资料版本已不可恢复，请重新生成。",
    contextBudgetExceeded: "当前上下文配置无法容纳系统指令和问题，请降低输出上限或提高上下文容量。",
    generationOutputRejected: "服务商拒绝了本次输出长度设置，请降低最大输出长度后重试。",
    outputLimitReached: "已达到本次输出上限（{{limitTokens}} tokens），回答可能未完成。可继续生成、重新生成，或在编辑模型配置中提高最大输出长度。",
    modelRouteInconsistent: "已保存的模型路由不一致，请重新打开设置并保存两个模型。",
    network: "无法连接提供商，请检查 API 地址和网络后重试。",
    provider: "提供商返回无效响应或暂时不可用，请检查其服务状态后重试。",
    rateLimited: "提供商限制了请求频率，请稍等片刻后重试。",
    timeout: "提供商响应超时，请检查地址或网络后重试。",
    validation: "请检查提供商地址、模型名称和必填项。"
  },
  error: {
    loadProjects: "无法加载项目，请重试。",
    createProject: "无法创建项目。",
    renameProject: "无法重命名项目。",
    archiveProject: "无法归档项目。",
    removeProject: "无法删除项目。",
    restoreProject: "无法恢复项目。",
    retryDeleteProject: "无法重试删除项目。"
  },
  chat: {
    newConversation: "新对话",
    citeCollapse: "收起引用面板",
    citeExpand: "展开引用面板",
    emptyTitle: "针对来源开始提问",
    emptyBody: "你的问题会结合已建立索引的来源作答，并附上可追溯的引用。",
    copied: "已复制到剪贴板。",
    roleUser: "提问",
    roleAssistant: "回答",
    message: {
      streaming: "回答中…",
      completed: "已回答",
      cancelled: "已停止。",
      failed: "回答失败。"
    },
    repair: {
      retry: "重试",
      repair: "修复回答"
    },
    renameHint: "单击切换对话，双击重命名",
    thinking: "思考等级",
    thinkingHint: "等级越高回答越严谨但越慢；不支持思考等级的模型会自动降级",
    thinkingLevel: { off: "关闭", low: "低", medium: "中", high: "高" },
    retryBusyHint: "正在生成回答，请等待完成或停止后再重试",
    ui: {
      outputLimitUnknown: "已达到本次输出上限，回答可能未完成。",
      conversations: "对话", rename: "重命名", remove: "删除", saveTitle: "保存对话标题", noConversations: "暂无对话。", conversationTitle: "对话标题", viewDetails: "查看原文",
      citationTitle: "来源引用", citationEmpty: "回答中的引用会显示在这里。", sourceExcerptTitle: "引用原文", sourceExcerptUnavailable: "原文暂不可用。", openOriginal: "打开原始来源", opened: "已打开{{kind}}", sourceLabel: "已有资料", indexed: "已建立索引", indexedLabel: "已建立索引", processingComplete: "资料处理完成", research: "研究", researchChat: "研究对话", askPlaceholder: "针对来源提问，Enter 发送，Shift+Enter 换行…", model: "模型", generationModel: "生成模型", noModel: "无模型", send: "发送", stop: "停止", copy: "复制", regenerate: "重新生成", continueGeneration: "继续生成", openModelSettings: "模型设置", outputLimitReached: "已达到本次输出上限（{{limitTokens}} tokens），回答可能未完成。", continuationInterrupted: "续写已中断，回答已保留，可再次继续生成。", editAndResend: "编辑并重新发送", answerFailed: "回答失败。", retryAnswer: "重试", errorCode: "错误码：{{code}}", errorTarget: "目标模型：{{target}}", fallbackBanner: "首选模型暂时不可用，回答已切换到回退模型。", unavailableTitle: "研究对话尚未就绪", unavailableImport: "导入来源并等待索引完成后，即可开始提问。", unavailableModel: "请在设置中配置生成模型后提问。", importSources: "导入来源", openSettings: "打开设置", rebuildIndex: "重建索引", pending: "待处理", ready: "已就绪", sourceProgress: "{{stage}} · {{progress}}%", page: "第", slide: "幻灯片", sheet: "工作表", cell: "单元格"
    }
  },
  research: {
    workspaceTitle: "导入资料，开始你的研究",
    importSources: "导入来源",
    sources: "来源",
    collapsePanel: "收起来源面板",
    expandPanel: "展开来源面板",
    importSubtitle: "导入本地文件或网页，解析后自动建立索引。",
    importProgress: "正在导入 {{done}}/{{total}}…",
    noSourcesTitle: "暂无来源",
    noSourcesBody: "导入的文件或网页将在这里显示，并自动建立索引。",
    noSourcesShort: "还没有已建立索引的来源。",
    sourcesReadyCount: "已建立索引的来源：{{count}} 个。",
    ask: "针对这个项目提问",
    sourceImportUnavailable: "导入文件或网页，开始你的研究。",
    researchChatUnavailable: "导入资料后即可使用研究对话。",
    settingsUnavailable: "打开设置即可配置模型服务。",
    openSource: "打开来源",
    buildIndex: "构建索引",
    indexStuckHint: "来源长时间未完成索引？",
    chooseFiles: "选择文件",
    importUrl: "导入网址",
    webAddress: "网页地址",
    orDivider: "或",
    removeSource: "移除来源",
    indexed: "已建立索引",
    pending: "处理中",
    selected: "已选择",
    formats: "支持 PDF、DOCX、PPTX、XLSX、TXT、Markdown、URL、CSV",
    importing: "正在导入…",
    importError: "导入失败",
    task: { cancel: "取消任务", retry: "重试任务", remove: "移除任务", completed: "已完成", failed: "失败", queued: "排队中", running: "处理中", cancelled: "已取消", validating: "验证中", staging: "准备中", parsing: "解析中", chunking: "分块中", embedding: "嵌入中", indexing: "建立索引中", verifying: "校验中", cleanup: "清理中", finalizing: "收尾中", preparing: "准备中", generating: "生成中", saving: "保存中" }
  },
  notes: {
    eyebrow: "项目笔记",
    titlePage: "笔记",
    subtitle: "用 Markdown 记录和整理有来源依据的研究。",
    new: "新建笔记",
    archived: "已归档笔记",
    archivedTag: "已归档",
    active: "当前笔记",
    count: "篇笔记",
    empty: "暂无笔记",
    emptyBody: "笔记支持 Markdown 与实时预览，可以关联来源作为证据。",
    select: "在左侧选择一篇笔记，或新建一篇。",
    actions: "笔记操作",
    collapsePanel: "收起笔记面板",
    expandPanel: "展开笔记面板",
    renameHint: "双击重命名",
    editorEyebrow: "笔记编辑器",
    edit: "编辑笔记",
    title: "标题",
    body: "Markdown 正文",
    bodyPlaceholder: "用 Markdown 记录研究心得…",
    preview: "预览",
    save: "保存笔记",
    savedToast: "笔记已保存。",
    unsaved: "未保存",
    aiTitle: "生成 AI 标题",
    aiTitleGenerating: "正在生成标题…",
    archive: "归档",
    restore: "恢复",
    delete: "删除",
    deleteConfirm: "确认永久删除这篇笔记？",
    conflict: "笔记已在其他位置发生变化。请重新加载服务端版本，或保留本地草稿。",
    reloadServer: "重新加载服务端版本",
    keepDraft: "保留本地草稿",
    titleFailure: "AI 标题生成失败，原标题和正文已保留。",
    links: "关联证据",
    linkType: "关联类型",
    project: "项目",
    source: "来源",
    message: "消息",
    questionMessage: "提问消息",
    modelAnswer: "模型回答",
    citation: "引用",
    target: "目标 ID",
    targetPlaceholder: "粘贴目标 ID",
    linkSource: "选择来源",
    linkSourcePlaceholder: "选择一个来源进行关联…",
    linkProject: "选择项目",
    linkProjectPlaceholder: "选择一个项目…",
    noProjectSources: "该项目暂无可关联资料",
    noMessages: "暂无可关联消息",
    linkTargets: "选择关联内容",
    multiSelectHint: "按住 Ctrl 可选择多个内容。",
    link: "关联",
    linking: "关联中…",
    selectedCount: "已选 {count} 项",
    unlink: "移除关联",
    targetUnavailable: "目标不可用",
    noLinks: "还没有关联证据。",
    untitled: "未命名笔记",
    unavailable: "笔记服务尚未就绪。"
  },
  workspace: {
    research: "研究",
    studio: "转换",
    subtitle: "导入来源、提问并沉淀笔记"
  },
  transformations: {
    podcastSynthetic: "AI 生成的双人对谈音频",
    podcastPlayer: "播客播放器",
    eyebrow: "研究工具",
    title: "转换",
    subtitle: "将项目内容转换为可复用的洞察。",
    runTitle: "运行转换",
    rule: "规则",
    summary: "摘要",
    builtins: "内置规则",
    custom: "自定义规则",
    target: "目标类型",
    targetId: "目标 ID",
    targetPlaceholder: "来源、提问消息或模型回答",
    targetProject: "项目",
    targetSource: "来源",
    targetMessage: "提问消息",
    targetAnswer: "模型回答",
    projectSources: "将读取当前项目中全部 {{count}} 个已完成索引的来源。",
    pickSource: "选择一个来源…",
    pickMessage: "选择具体消息",
    pickMessagePlaceholder: "选择当前项目中的一条消息…",
    converted: "已转换为笔记。",
    run: "运行转换",
    cancel: "取消",
    retry: "重试",
    customRules: "自定义规则",
    newRule: "新建自定义规则",
    practice: "互动答题",
    noQuestions: "暂无题目",
    practiceHint: "运行“选择题测验”规则，即可基于资料生成一套互动选择题。",
    questionSets: "练习题组",
    questionSet: "题组",
    singleChoice: "单选题",
    correct: "回答正确",
    incorrect: "回答错误",
    correctAnswerIs: "正确答案是",
    explanation: "答案解析与依据",
    tryAgain: "重做本题",
    retake: "重新测试",
    viewSummary: "查看测试成绩",
    backToQuiz: "返回题目",
    scoreRate: "正确率",
    scoreCount: "答对 {{correct}} / {{total}} 题",
    perfectScore: "太棒了！全部回答正确 🎉",
    greatScore: "掌握得非常扎实！继续保持 👏",
    passScore: "基本掌握，建议对照错题复习 📖",
    lowScore: "建议重点研读资料后重新测试 💪",
    unanswered: "未作答",
    practiceAccuracy: "正确率 {{rate}}%",
    yourAnswer: "写下你的答案，再对照参考答案自评…",
    revealAnswer: "查看参考答案",
    referenceAnswer: "参考答案",
    selfAssess: "自评",
    known: "已掌握",
    review: "需复习",
    practiceProgress: "已完成 {{count}} / {{total}} 题",
    unreadableQuestions: "这套测验暂未识别出完整题目，请查看洞察详情，或重新运行选择题测验。",
    practiceSaveError: "答题进度未能保存到本机，请暂时不要关闭页面。",
    editRule: "编辑自定义规则",
    ruleName: "规则名称",
    appliesTo: "适用目标",
    targetKinds: { sources: "项目", source: "来源", message: "提问消息", answer: "模型回答", note: "笔记（旧版）" },
    prompt: "提示词",
    placeholders: "允许的占位符",
    placeholderHelp: "HTML 和其他模板语法会被拒绝。",
    invalidPrompt: "只能使用四个受支持的占位符。",
    ruleNameRequired: "请输入规则名称。",
    noRules: "还没有自定义规则。",
    insights: "洞察",
    noInsights: "已完成的洞察会显示在这里。",
    viewDetails: "查看详细",
    insightDetail: "洞察详情",
    podcastDetail: "播客详情",
    convert: "转换为笔记",
    unavailable: "转换服务尚未就绪。",
    elapsed: "已用 {{seconds}} 秒",
    phases: { preparing: "准备资料", connecting: "模型响应中", generating: "正在生成内容", saving: "正在整理结果" },
    podcastPhases: { preparing: "准备播客资料", script: "生成双人对话稿", speech: "合成双人语音", saving: "保存播客音频" },
    states: { queued: "排队中", running: "处理中", completed: "已完成", failed: "失败", cancelled: "已取消" }
  }
} as const;

export default zhCN;
