...
export default function ChatView({ projectId, routes, sources: suppliedSources, api: suppliedApi, onOpenSettings, onImportSources, onRebuildIndex, sourceRefreshKey }: ChatViewProps) {
  ...
  const taskBySource = useMemo(() => { const latest = new Map<string, TaskDto>(); for (const task of tasks) if (task.sourceId && !latest.has(task.sourceId)) latest.set(task.sourceId, task); return latest; }, [tasks]);
  ...
  async function retrySource(sourceId: string): Promise<void> { const result = await window.myNotebook?.sources?.retry?.({ projectId, sourceId }); if (result?.ok) setTasks((current) => [result.value, ...current.filter((task) => task.sourceId !== sourceId)]); }
  ...
  const sourceStrip = <div className="chat-source-strip" aria-label={t("chat.ui.sourceLabel", "Sources")}><div className="chat-source-row"><div className="chat-source-summary"><span className="eyebrow">{t("chat.ui.sourceLabel", "Sources")}</span><strong>{sources.filter(sourceReady).length} {t("chat.ui.indexed", "indexed")}</strong></div>{sources.map((source) => { const ready = sourceReady(source); const task = taskBySource.get(source.id); const failed = source.currentRevisionState === "failed" || task?.state === "failed"; const sizeBytes = (source as SourceDto & { sizeBytes?: number }).sizeBytes; const metadata = source.kind === "url" ? source.locator ?? source.displayName : `${sourceFormatLabel(source.kind)}${sizeBytes === undefined ? "" : ` · ${formatBytes(sizeBytes)}`}`; return <button type="button" className={`source-chip${ready ? " ready" : failed ? " failed" : ""}`} key={source.id} onClick={() => void window.myNotebook?.sources?.open?.({ projectId, sourceId: source.id })}><SourceKindIcon kind={source.kind} className="chat-source-kind-icon" /><span className="source-chip-copy"><strong>{source.displayName}</strong><small>{metadata}</small><small className="source-chip-indexed"><span aria-hidden="true">{ready ? "✓" : failed ? "!" : ""}</span>{ready ? t("chat.ui.indexedLabel", "Indexed") : failed ? t("sources.task.failed", "Failed") : t("chat.ui.pending", "pending")}</small></span></button>; })}{onImportSources ? <button type="button" className="source-import-action" onClick={onImportSources}><span aria-hidden="true">＋</span>{t("chat.ui.importSources", "Import sources")}</button> : null}</div>{tasks.filter((task) => task.state === "queued" || task.state === "running").map((task) => <span className="chat-source-progress" role="status" key={task.id}><span>{t("chat.ui.sourceProgress", "{{stage}} · {{progress}}%", { stage: task.stage, progress: Math.round(task.progress / 10) })}</span><progress max={100} value={Math.round(task.progress / 10)} aria-label={t("chat.ui.sourceProgress", "{{stage}} · {{progress}}%", { stage: task.stage, progress: Math.round(task.progress / 10) })} /></span>)}{tasks.filter((task) => task.sourceId && task.state === "failed").map((task) => <div className="chat-source-failure" role="alert" key={task.id}><span>{t("sources.task.failed", "Failed")} · {t(`sources.task.${task.stage}`, task.stage)}</span><button type="button" onClick={() => void retrySource(task.sourceId!)}>{t("sources.task.retry", "Retry")}</button></div>)}{sources.length > 0 && sources.every(sourceReady) ? <div className="chat-source-complete" role="status"><span aria-hidden="true">✓</span><strong>{t("chat.ui.processingComplete", "Processing complete")}</strong><span>{sources.length} {t("chat.ui.indexedLabel", "Indexed")}</span></div> : null}</div>;
