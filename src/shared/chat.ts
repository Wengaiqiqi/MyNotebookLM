...
export const generationFinishKindSchema=z.enum(["stop","length","context-limit","other"]);
export const generationSummarySchema=z.object({
  revision:z.number().int().nonnegative(),
  status:z.enum(["idle","running","interrupted"]),
  finishKind:generationFinishKindSchema.nullable(),
  outputTokenLimit:z.number().int().positive().nullable(),
  canContinue:z.boolean(),
  blockedReason:z.string().nullable(),
  lastError:z.string().nullable(),
  usageComplete:z.boolean()
}).strict();
export const citationLabelSchema=z.string().regex(/^S[1-9]\d{0,15}$/).refine((value) => Number.isSafeInteger(Number(value.slice(1))), "Citation id must be a safe positive integer");
