...
describe("parsePptx", () => {
  it("does not follow external or escaping notes relationships", async () => {
    const zip = new JSZip();
    zip.file("ppt/slides/slide1.xml", '<p:sld xmlns:p="p"><a:t xmlns:a="a">Title</a:t></p:sld>');
    zip.file("ppt/slides/_rels/slide1.xml.rels", '<Relationships><Relationship Type="notesSlide" Target="../../evil.xml"/></Relationships>');
    zip.file("evil.xml", '<a:t xmlns:a="a">SECRET</a:t>');
    const blocks = await parsePptx(await zip.generateAsync({ type: "uint8array" }));
    expect(blocks).toEqual([{ kind: "heading", text: "Title", locator: { kind: "slide", slide: 1 } }]);
  });
