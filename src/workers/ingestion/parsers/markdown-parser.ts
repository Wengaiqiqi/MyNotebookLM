...
export function parseMarkdown(input: string | Uint8Array): DocumentBlock[] {
  ...
  for (let i = 0; i < lines.length;) { const line = lines[i]!; const match = /^(#{1,6})\s+(.+)$/.exec(line);
    if (match) { const depth = match[1]!.length; headingPath = headingPath.slice(0, depth - 1); headingPath.push(match[2]!.trim()); const path = headingPath.join(" > "); add("heading", match[2]!, { kind: "heading", depth, headingPath: path }); i++; offset += line.length + 1; continue; }
    ...
    if (line.trim()) { const paragraphLines = [line.trim()]; const startParagraph = ++paragraph; i++; offset += line.length + 1; while (i < lines.length && lines[i]!.trim() && !/^(#{1,6})\s+/.test(lines[i]!) && !lines[i]!.startsWith("```") && !/^\s*[-*+]\s+/.test(lines[i]!)) { paragraphLines.push(lines[i]!.trim()); offset += lines[i]!.length + 1; i++; } add("paragraph", paragraphLines.join("\n"), { kind: "paragraph", paragraph: startParagraph }); continue; } i++; offset += line.length + 1;
  } return blocks;
}
