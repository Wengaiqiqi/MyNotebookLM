...
export function parseMarkdown(input: string | Uint8Array): DocumentBlock[] {
  ...
  for (let i = 0; i < lines.length;) { const line = lines[i]!; const match = /^(#{1,6})\s+(.+)$/.exec(line);
    ...
    if (line.includes("|") && i + 1 < lines.length && /^\s*\|?\s*:?-+:?/.test(lines[i + 1]!)) { const start = offset; const rows = [line]; offset += line.length + 1; i++; rows.push(lines[i]!); offset += lines[i]!.length + 1; i++; while (i < lines.length && lines[i]!.includes("|")) { rows.push(lines[i]!); offset += lines[i]!.length + 1; i++; } add("table", rows.join("\n"), { kind: "offset", start, end: offset }); continue; }
    if (line.trim()) { const text = line.trim(); paragraph++; add("paragraph", (headingPath.length ? headingPath.join(" > ") + "\n" : "") + text, { kind: "paragraph", paragraph }); } i++; offset += line.length + 1;
  } return blocks;
}
