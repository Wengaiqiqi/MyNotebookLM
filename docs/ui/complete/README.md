...
### Task 5 研究对话书面方向（2026-08-28）

- 保留 Foundation 左侧项目导航与连续桌面窗口；研究区上方呈现已导入来源卡片及处理进度，中部为带编号引用的对话，右侧固定“来源引用”详情卡片，编号与回答中的引用一一对应。
- 底部使用大圆角多行输入框，操作集中在右下：保留模型选择；仅当实际路由能力支持时显示推理强度；发送后同一位置切换为停止状态。
- 参考图左侧的“+”与权限控制不实现。不得添加无真实后端能力的装饰性控制。
- 这是 Task 5 的实现方向，不提前视为 Gate F 最终截图批准；完成后仍须用真实 Electron `zh-CN/light` 与 `en/dark` 截图书面验收。
