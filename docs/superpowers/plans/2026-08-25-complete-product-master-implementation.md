...
- [x] **Gate G — final release:** Plan 6's complete checks pass before `package:win`; the generated installer is then installed and tested without rebuilding; its SHA-256 remains identical to the delivered file.
  ...
  Evidence (2026-08-30, candidate 2): candidate 1 failed because `apache-arrow` (a peerDependency of `@lancedb/lancedb`) was missing from the packaged dependency closure — the packaged exe showed a main-process "Cannot find module 'apache-arrow'" dialog and never opened a window; the earlier migrations-path diagnosis was an artifact of reproducing with the dev Electron binary and was disproved. Fix: declared `apache-arrow@18.1.0` in app `dependencies` with a RED->GREEN peer-closure regression test (`src/main/db/packaged-dependencies.test.ts`); fresh gates all green (unit 108 files/997 tests, typecheck, build, e2e 20/20) and a new candidate was packaged. Full install smoke 18/18 PASS including restart persistence, overwrite-install retention, uninstall keeping user data (`deleteAppDataOnUninstall: false`) and reinstall opening retained data. Installer: `dist/MyNotebookLM-Setup-1.0.0.exe`, 360,399,111 bytes, SHA-256 `2F4A8071EEB053F0AE72DC2DF7F33DC11554DB94DF4916AE478577BF346D599C`, unchanged after smoke testing; unsigned, SmartScreen warning expected; no clean Windows machine available (recorded honestly). Full record in `docs/release/2026-08-25-verification.md` Task 10 re-verification section.
