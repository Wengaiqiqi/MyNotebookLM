...
- [ ] Write failing migration/repository tests for default onboarding state, locale/theme persistence, profile CRUD, unique profile IDs, generation/embedding capability validation and route ordering.
- [ ] Add migration tables `app_settings`, `model_profiles`, `model_routes`, `credentials`; use ISO timestamps, foreign keys, unique route position and `CHECK` constraints for enumerations.
- [ ] Add strict Zod schemas and infer DTOs from them. Provider kinds are `openai`, `openai-compatible`, `anthropic`, `gemini`, `ollama`, `local`; capabilities are `generation`, `embedding`.
- [ ] Implement SQL-only repository methods and transactions; do not decrypt credentials here.
- [ ] Run `npm test -- --run src/main/db/database.test.ts src/main/settings/settings-repository.test.ts`; expect all focused tests green.
- [ ] Run `npm run typecheck`, review and commit `feat: add settings and model persistence`.
...
- [ ] Write failing tests proving encrypted bytes differ from input, replacing/removing works, empty secrets are rejected, masks reveal no secret characters and secret text never appears in thrown errors.
- [ ] Adapt Electron async `safeStorage`; fail closed when encryption is unavailable. Unit tests inject a deterministic fake protector, never Electron itself.
- [ ] Store encrypted blobs in `credentials`; do not expose a general `getSecret` method to IPC.
- [ ] Wire the store after `app.whenReady()` and close repositories during shutdown.
- [ ] Run `npm test -- --run src/main/credentials`; expect all tests green, then `npm run typecheck`.
- [ ] Review and commit `feat: protect model credentials with safe storage`.
...
- [ ] Write failing tests for timeout, explicit abort, 401/403 auth, 404 invalid model, 429 retry metadata, 5xx recoverable provider error, malformed JSON and secret redaction.
- [ ] Implement one native-fetch helper with URL joining, response size limit, JSON/SSE/NDJSON readers and `AbortSignal.any` timeout composition.
- [ ] Implement one central classifier returning `AppErrorDto`; authentication/config/cancel are non-fallback errors, network/timeout/429/5xx are fallback-eligible metadata.
- [ ] Run `npm test -- --run src/main/models/http-client.test.ts src/main/models/provider-errors.test.ts`; expect green.
- [ ] Review and commit `feat: define provider transport contract`.
...
- [ ] Start a loopback fake HTTP server in tests and first assert failures for bearer authentication, discovery mapping, SSE text/usage/done events, batch embeddings, malformed vectors, abort and all classified status codes.
- [ ] Implement direct REST calls. Default OpenAI address is `https://api.openai.com/v1`; OpenAI-compatible uses the user-editable address.
- [ ] Validate that every embedding vector is finite, non-empty and has identical dimension; reject mixed dimensions.
- [ ] Run `npm test -- --run src/main/models/openai-provider.test.ts`; expect green.
- [ ] Review and commit `feat: connect openai compatible providers`.
...
- [ ] Write fake-server contract tests for exact headers, model mapping, streaming event mapping, embeddings where supported, abort and errors.
- [ ] Implement Anthropic with `x-api-key` and `anthropic-version`; `embed` returns a typed unsupported-capability error before network I/O.
- [ ] Implement Gemini with `x-goog-api-key`; filter discovery by supported generation/embedding methods.
- [ ] Implement Ollama NDJSON chat and array embeddings. Default address is `http://127.0.0.1:11434`; no key is required for this local default.
- [ ] Run `npm test -- --run src/main/models/*-provider.test.ts`; expect all provider contracts green.
- [ ] Review and commit `feat: connect anthropic gemini and ollama`.
...
- [ ] Write failing tests for immutable provider/address/model/dimension metadata and no-credential behavior.
- [ ] Add the built-in descriptor and disallow editing its base URL/key while allowing selection as embedding default.
- [ ] Run focused tests and typecheck; review and commit `feat: register built in embedding model`.
...
- [ ] Write failing service/IPC/preload tests for runtime validation, provider construction, discovery, save-after-test, manual model name, secret-preserving edits and unsubscribe cleanup.
- [ ] Implement service orchestration. `test` performs discovery when possible and a minimal capability call only when the manual model cannot be verified from discovery.
- [ ] Ensure provider addresses accept only `http:`/`https:` and reject embedded credentials; allow loopback only for Ollama or explicitly selected OpenAI-compatible local endpoints.
- [ ] Register handlers once, remove them on teardown and validate both directions with Zod.
- [ ] Run focused tests, full preload tests and typecheck; review and commit `feat: expose model settings api`.
...
- [ ] Write failing tests for `titleBarStyle: 'hidden'`, enabled `titleBarOverlay`, secure web preferences and renderer drag/no-drag classes.
- [ ] Update window creation so native `—`, `□`, `×` are embedded into the renderer background. Update overlay symbol color when theme changes through a dedicated validated IPC command.
- [ ] Render every dialog under one root-level portal into `document.body`; backdrop uses `position: fixed; inset: 0`; modal uses grid centering over the full viewport.
- [ ] Add an E2E geometry assertion: modal center is within 2 CSS pixels of viewport center at default and resized window dimensions.
- [ ] Run window/App tests, Electron E2E and typecheck; review and commit `fix: integrate title controls and center dialogs`.
...
- [ ] Write failing component tests for initial routing, provider defaults, editable address, conditional API key, discovery success timing, dropdown/manual entry, validation, skip, save, later settings edits and keyboard/focus behavior.
- [ ] Split only the model form/view responsibilities out of `App.tsx`; keep ordinary React state and effects, no state library.
- [ ] Implement the approved `first-launch.png` and `settings-models.png`; settings sidebar button must actually open the view.
- [ ] On skip, persist onboarding completion and make every model-dependent feature show an “open settings” repair action.
- [ ] Add identical zh-CN/en keys and a test that recursively compares locale key sets.
- [ ] Run component tests and E2E with fake preload. Capture `docs/verification/screenshots/onboarding-zh-light.png` and `settings-en-dark.png` from real Electron.
- [ ] Compare dimensions, centering, control placement and states to approved images; correct differences before review.
- [ ] Run Gate A from the master plan; review and commit `feat: complete model onboarding and settings`.
