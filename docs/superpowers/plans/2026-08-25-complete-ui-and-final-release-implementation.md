...
- [x] Add real Electron screenshots for the research-chat state under `docs/verification/screenshots/` and compare them against the approved direction. Final evidence (2026-08-29): three real PDFs remain loaded in the real user-data profile; revised `research-chat-zh-light.png` and `research-chat-en-dark.png` are complete 1803x1128 Electron views with `scrollY=0`, the full header/source/chat/composer surface, and citation/composer bottom-edge delta of 0.20px. Duplicate top-level S1/S2 indexes were removed; answer citations are compact clickable chips that switch to the owning assistant message's citation set, scroll and highlight the right card, including when separate answers reuse S1. New citations persist at most 240 characters of trusted retrieved chunk text as the detail quote, while existing null quotes remain honest. Fallback status is scoped and centered below its answer. Independent read-only review PASS; full verification: 101 files / 931 tests, typecheck, production build and diff-check passed.
- [x] Review and commit `feat: complete cited research chat interface`.
...
- [x] Run the final prepackage suite once more: `npm test`, `npm run typecheck`, `npm run build`, `npm run test:e2e`. Stop on any failure. Evidence: all exit 0 — 107 files / 996 tests, typecheck clean, build clean, e2e 20/20 (one intermittent test-infra flake fixed via retry helper; product code unchanged).
- [x] Run `npm run package:win` exactly once per candidate. From this point, do not rebuild or edit packaged inputs. Evidence: candidate 1 (2026-08-30) exit 0; after the root-cause fix below, candidate 2 packaged with exit 0, replacing the failed candidate (allowed failure-fix-repackage flow; two aborted interim attempts before any artifact change: one EBUSY from a stale diagnostic process, one network ETIMEDOUT).
- [x] Record installer byte size and `Get-FileHash -Algorithm SHA256` immediately. Evidence: candidate 1: 359,802,370 bytes, SHA-256 `9522DB07D5540788E0254DBED35ACC5DCBB44FB803241B5795750E5298F3EEEF` (failed in smoke, superseded). Candidate 2 (delivered): 360,399,111 bytes, SHA-256 `2F4A8071EEB053F0AE72DC2DF7F33DC11554DB94DF4916AE478577BF346D599C`.
- [x] Fresh-install the exact installer for the current user, launch with an isolated fresh user-data directory, confirm onboarding, and execute the representative complete research chain using test-safe local providers without developer server/runtime. Evidence: candidate 2 full PASS via `scripts/installer-smoke.mjs` (silent install, onboarding visible/skip, lancedb health, chat "Grounded alpha answer [S1]" with citation S1 resolved and opened, note created) — the packaged exe now opens its window; candidate 1 could not launch (see fix note).
- [x] Close the app, relaunch and verify projects, sources, conversations, notes, settings and active Space persist. Evidence: candidate 2 PASS (restart-assistant, restart-note, restart-active-space with spaceHealthy=true and indexedCount=1, restart-settings).
- [x] Run the same installer again over the installed copy and verify preserved user data. If an available signed/known `0.1.0` foundation installer is used for true upgrade testing, record its hash separately and never deliver it. Evidence: candidate 2 PASS (overwrite-install-ok, overwrite-data-retained).
- [x] Invoke the installed uninstaller, verify binaries/shortcuts are removed and user data remains because `deleteAppDataOnUninstall: false`; reinstall the same final installer and verify retained data can be opened. Evidence: candidate 2 PASS (uninstall-removed-binaries, uninstall-user-data-kept, reinstall-data-openable) — retention verified by actual uninstaller behavior, not just static config.
- [x] Hash the original final installer again and assert it exactly equals the pre-smoke SHA-256. Do not run package/build afterward. Evidence: candidate 2 post-smoke hash identical (`2F4A8071…599C`); no package/build run after packaging.
- [x] Finish the evidence ledger, including whether an independent clean Windows machine was actually available. Never substitute a same-machine isolated profile for that claim. Evidence: `docs/release/2026-08-25-verification.md` Task 10 re-verification section; explicitly records NO independent clean Windows machine was available.
- [x] Review release evidence and commit only source/docs changes that predate packaging. The installer remains the immutable delivery artifact. Evidence: fix + docs commits on `codex/rag-pipeline-fixes` after candidate 2 passed; installer excluded from git via dist ignore rules. Root-cause fix note: candidate 1 failed because `apache-arrow` (peerDependency of `@lancedb/lancedb`) was not in the packaged dependency closure; fixed by declaring `apache-arrow@18.1.0` in app dependencies with a peer-closure regression test; the earlier migrations-path diagnosis was disproved (migrations layout was always correct).
